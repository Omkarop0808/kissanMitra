...
export default function WeatherAdvisory() {
  ...
  const fetchWeather = async (searchCity: string) => {
    ...
    try {
      ...
      if (!currentRes.ok) {
        let errMsg = `Weather API error (${currentRes.status})`
        try { const e = await currentRes.json(); errMsg = e.detail || e.message || errMsg } catch {}
        throw new Error(errMsg)
      }
      if (!forecastRes.ok) {
        let errMsg = `Forecast API error (${forecastRes.status})`
        try { const e = await forecastRes.json(); errMsg = e.detail || e.message || errMsg } catch {}
        throw new Error(errMsg)
      }
