...
async function request(url: string, options?: RequestInit) {
  ...
  if (!res.ok) {
    ...
    } catch { /* response was not JSON (e.g. 502 HTML page) */ }
...
export const diseaseApi = {
  predict: async (formData: FormData) => {
    const r = await fetch('/disease_prediction', { method: 'POST', body: formData })
    if (!r.ok) {
      let detail = `Server error (${r.status})`
      try { const e = await r.json(); detail = e.detail || e.message || detail } catch {}
      throw new Error(detail)
    }
    return r.json()
  },
  geminiAnalyze: async (formData: FormData) => {
    const r = await fetch('/api/gemini-analyze', { method: 'POST', body: formData })
    if (!r.ok) {
      let detail = `Server error (${r.status})`
      try { const e = await r.json(); detail = e.detail || e.message || detail } catch {}
      throw new Error(detail)
